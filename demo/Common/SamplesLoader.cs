...
public static class SamplesLoader
{
    ...
    /// </summary>
    private static readonly List<HtmlSample> _showcaseSamples = [];

    /// <summary>
    /// Samples to test the different features of HTML Renderer that they work correctly
    /// </summary>
    private static readonly List<HtmlSample> _testSamples = [];

    /// <summary>
    /// Samples used to test extreme performance
    /// </summary>
    private static readonly List<HtmlSample> _performanceSamples = [];
    ...
    private static void LoadSamples(string platform, string version)
    {
        ...
        foreach (string name in names)
        {
            ...
            string ext = name[(extPos >= 0 ? extPos : 0)..];
