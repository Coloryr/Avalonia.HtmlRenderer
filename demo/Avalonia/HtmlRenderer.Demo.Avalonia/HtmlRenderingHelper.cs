...
internal static class HtmlRenderingHelper
{
    ...
    /// </summary>
    private static readonly Dictionary<string, Bitmap> _imageCache = new(StringComparer.OrdinalIgnoreCase);
    ...
    public static void ImageLoad(HtmlImageLoadEventArgs e)
    {
        ...
        if (!e.Handled && e.Attributes != null)
        {
            ...
            {
                if (int.TryParse(e.Attributes["byevent"], out int delay))
                {
                    e.Handled = true;
            ...
            else if (e.Attributes.ContainsKey("byrect"))
            {
                ...
                var rect = new Rect(int.Parse(split[0]), int.Parse(split[1]), int.Parse(split[2]), int.Parse(split[3]));
