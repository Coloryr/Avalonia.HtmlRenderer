...
public partial class MainControl : UserControl
{
    ...
    private void LoadSamples()
    {
        ...
        if (SamplesLoader.PerformanceSamples.Count > 0)
        {
            ...
            {
                Header = PerformanceSamplesTreeNodeName
            };
    ...
    private void AddTreeItem(TreeViewItem root, HtmlSample sample)
    {
        ...
        {
            Header = sample.Name,
            Tag = new HtmlSample(sample.Name, sample.FullName, html)
        };
        root.Items.Add(node);
    }
    ...
    private void OnTreeView_OnSelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        if (item?.Tag is HtmlSample sample)
        {
            _updateLock = true;
    ...
    private static void MessageBox(string text, string? title = null)
    {
