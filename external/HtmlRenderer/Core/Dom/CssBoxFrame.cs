...
/// </summary>
internal sealed partial class CssBoxFrame : CssBox
{
    ...
    private static async Task<string> DiscoverOEmbedEndpointAsync(Uri uri)
    {
        try
        {
            ...
            var match = BoxFrameRegex().Match(html);
    ...
    /// </summary>
    private static string GetKnownProviderEndpoint(Uri uri)
    {
    ...
    {
        _imageLoadHandler = new ImageLoadHandler(HtmlContainer, OnLoadImageComplete);
        _imageLoadHandler.LoadImage(source, HtmlTag?.Attributes);
    }
    ...
    [GeneratedRegex(@"<link\s+[^>]*rel\s*=\s*[""']alternate[""'][^>]*type\s*=\s*[""']application/json\+oembed[""'][^>]*href\s*=\s*[""']([^""']+)[""'][^>]*>|<link\s+[^>]*type\s*=\s*[""']application/json\+oembed[""'][^>]*rel\s*=\s*[""']alternate[""'][^>]*href\s*=\s*[""']([^""']+)[""'][^>]*>")]
    private static partial Regex BoxFrameRegex();
