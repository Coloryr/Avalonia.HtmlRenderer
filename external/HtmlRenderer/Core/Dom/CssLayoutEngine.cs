...
internal static class CssLayoutEngine
{
    ...
    public static void MeasureImageSize(CssRectImage imageWord)
    {
        ...
        var cont = imageWord.OwnerBox.HtmlContainer.MaxSize.Width;
        if (maxWidth.Number > 0 || cont > 0)
        {
            ...
            {
                maxWidthVal = Math.Min(cont, maxWidthVal);
            }

            if (maxWidthVal > -1 && imageWord.Width > maxWidthVal)
            {
                double ratio = maxWidthVal / imageWord.Width;
                imageWord.Height = height.Number * ratio;
                imageWord.Width = maxWidthVal;
                scaleImageHeight = true;
                hasImageTagHeight = false;
            }
        }
        ...
        else if(scaleImageHeight == false)
        {
